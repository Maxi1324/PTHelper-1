...
class IPDialog(QDialog):
    ...
    def initUI(self):
        global ips
        self.setWindowTitle('ACL Changes')
        ...
        text = QLabel("Here you can change all ACL configs to another")
    ...
    def onReturnPressed(self, editor, ip):
        ...
        if changes[ip]["sv"] != changes[ip]["ip"]:
            editor.setText("ACEL changed from{} to {}".format(changes[ip]["ip"], changes[ip]["sv"]))
            changes[changes[ip]["ip"]] = changes[ip]["sv"] 
        else:
            editor.setText("Acl: {}".format(changes[ip]["ip"]))
...
def manipulate(xml):
    ...
    for key, v in changes.items():
        if not isinstance(v,str):
            newVal = v["sv"]
            xml_text = xml_text.replace("traffic-filter "+v["ip"], "traffic-filter "+newVal)
            xml_text = xml_text.replace("access-group name "+v["ip"], "access-group name "+newVal)
            xml_text = xml_text.replace("traffic-filter input "+v["ip"], "traffic-filter input "+newVal)
          #  xml_text = xml_text.replace("list "+v["ip"], "list "+newVal)
...
def settings(xml):
    ...
    pattern = re.compile(r'(ipv6 access-list (extended|standard) \S+)')
    ips = set(re.findall(pattern, xml_text))
    ips = (ip[0]+ip[1] for ip in ips)
    ips = sorted(ips)
