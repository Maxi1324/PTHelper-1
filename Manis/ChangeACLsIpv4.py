...
class IPDialog(QDialog):
    ...
    def onReturnPressed(self, editor, ip):
        ...
        if changes[ip]["sv"] != changes[ip]["ip"]:
            editor.setText("ACEL number changed from{} to {}".format(changes[ip]["ip"], changes[ip]["sv"]))
            changes[changes[ip]["ip"]] = changes[ip]["sv"] 
        else:
            editor.setText("Acl Number: {}".format(changes[ip]["ip"]))
...
def manipulate(xml):
    ...
    for key, v in changes.items():
        if not isinstance(v,str):
            ...
            xml_text = xml_text.replace("access-group "+v["ip"], "access-group "+newVal)
...
def settings(xml):
    ...
    pattern = re.compile(r'(access-list \d+|ip access-list (extended|standard) \S+)')
