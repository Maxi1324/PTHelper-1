...
class IPDialog(QDialog):
    ...
    def initUI(self):
        global ips
        self.setWindowTitle('Change OSPF Process IDs')
        ...
        text = QLabel("Here you can change all ospf Process Ids")
    ...
    def onReturnPressed(self, editor, ip):
        ...
        if changes[ip]["sv"] != changes[ip]["ip"]:
            editor.setText("OSPF Process Id changed from {} to {}".format(changes[ip]["ip"], changes[ip]["sv"]))
            changes[changes[ip]["ip"]] = changes[ip]["sv"] 
        else:
            editor.setText("OSPF Process Id: {}".format(changes[ip]["ip"]))
...
def manipulate(xml):
    ...
    for key, v in changes.items():
        if not isinstance(v,str):
            newVal = v["sv"]
            replaceStr = key.replace("ospf "+v["ip"], "ospf "+newVal)
