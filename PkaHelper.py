...
class PkaHelper:
    ...
    def __init__(self, container_name="hi8", image_name="jaja:3.0.0"):
        ...
        self.work_dir = os.path.join(os.getcwd(), "files")
        try:
            os.makedirs("files")
        except:
            pass
    ...
    def _pka2xml(self, url, source, result, mode, fp=None, native=False):
        if not native:
            self.container.start()
        if mode == "-d":
            shutil.copy(url, os.path.join(self.work_dir, source))
        else:
            with open(os.path.join(self.work_dir, source), 'wb') as f: 
                f.write(url)
        print("is native: "+str(native))
        if native:
            import sys
            p = os.getcwd()
            exe_path = os.path.join(p, "PkaHelper.exe")
            src_path = os.path.join("files", source)
            res_path = os.path.join("files", result)
            os.system(f"\"{exe_path}\" {mode} {src_path} {res_path}")
        else:
            src_path = os.path.join("files", source)
            res_path = os.path.join("files", result)
            self.container.exec_run(f"pka2xml {mode} {src_path} {res_path}", tty=True)
        
        if mode == "-d":
            with open(os.path.join(self.work_dir, result), 'rb') as f: 
                return f.read()
        if mode == "-e":
            shutil.copy(os.path.join(self.work_dir, result), fp)
